...
$(function() {
	...
	$("#submit").click(function() {
		...
		showMessage("송신 본문의 content-type : " + "text/plain");
		showMessage("송신 본문 내용  : " + $("#comment"));
		
			$.ajax({
				type : "post",
		        url : "/control",
		        dataType : "text/plain",
		        data : "2"+$("#comment").val()+"3",
		        error: function(error){ 
		        },
		        success : function(res){
	...
	$("#send").click(function() {
		...
		stompClient.send("/app/timeline", {}, $("#sample").val());
	});
});

function setConnected(connected) {
    $("#connect").css("visibility", connected ? 'hidden' : 'visible');
    $("#disconnect").css("visibility", connected ? 'visible' : 'hidden');
...
function connect() {
    ...
    stompClient.connect({}, function(frame) {
        ...
        stompClient.subscribe('/topic/subscribe', function(message){
    		// JSON.parse(message.body).headers
        	// {content-type=[application/vnd.onem2m-ntfy+xml], x-m2m-origin=[/indus], x-m2m-ri=[urn:cse:rqi:notif.495447895.1464398668943], x-m2m-ot=[20160528T102428], content-length=[1438], host=[192.168.181.3], connection=[Keep-Alive], user-agent=[Apache-HttpAsyncClient/4.1 (Java/1.7.0_80)]}
        	...
        	// graph value
        	light_value = JSON.parse(JSON.parse(message.body).body).value.light / 10;
        });
    }, function(error) {
    	console.log(error);
    	disconnect();
